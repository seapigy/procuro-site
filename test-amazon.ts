 ...
 * Run with: npx tsx test-amazon.ts
 */
...
async function testAmazonProvider() {
  ...
  if (!process.env.AMAZON_ACCESS_KEY || !process.env.AMAZON_SECRET_KEY) {
    ...
    console.log('\n💡 Note: You need to sign up for Amazon Product Advertising API:');
    console.log('   https://webservices.amazon.com/paapi5/documentation/');
    return;
  }
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('404')) {
        ...
        console.log('\n💡 Check your Amazon API credentials in server/.env');
